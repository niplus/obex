...
public class XianhuoWeituoPresenter extends BaseListPresenter<BaseListView> {
    private Long start_time = null;
    private Long end_time = null;

    public XianhuoWeituoPresenter(BaseListView view) {
        super(view);
    }
    ...
    public void onLoadData(final boolean isLoadMore) {
        ...
        Http.getHttpService().getUSDTTradeList(map)
                .compose(new CommonTransformer<XianhuoWeituoBean>())
                .subscribe(new CommonSubscriber<XianhuoWeituoBean>(getView()) {
                    @Override
                    public void onNext(XianhuoWeituoBean chengjiaoBean) {
                        if (getView() == null) {
                            return;
                        }
                    ...
                    protected void onError(ApiException e) {
                        super.onError(e);
                        L.a("xianhuoweituo", "xianhuoweituo fail" + e);
                        getView().stopProgressDialog();
                    }
