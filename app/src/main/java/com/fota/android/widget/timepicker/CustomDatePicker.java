package com.fota.android.widget.timepicker;
...
public class CustomDatePicker {
    ...
    public interface ResultHandler {
        void handle(long start_time, long end_time);
    }
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!startChecked) {
                    startChecked = true;
                    tv_title.setText(context.getString(R.string.timepicker_end));
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
                    startTime = selectedCalender.getTime().getTime();
                } else {
                    long end_time = selectedCalender.getTime().getTime();
                    if (end_time < startTime)
                        return;
                    startChecked = false;
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
                    selectedCalender.add(selectedCalender.DAY_OF_MONTH, 1);//选中的截止日期加1天到第二天0点
                    handler.handle(startTime, selectedCalender.getTime().getTime());
                    datePickerDialog.dismiss();
                }
            }
        });
    ...
    private void addListener() {
        year_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                String str = StringFormatUtils.delYDM(text);
                selectedCalender.set(Calendar.YEAR, Integer.parseInt(str));
                monthChange();
            }
        });

        month_pv.setOnSelectListener(new DatePickerView.onSelectListener() {//xiugai
            @Override
            public void onSelect(String text) {
                String str = StringFormatUtils.delYDM(text);
                selectedCalender.set(Calendar.DAY_OF_MONTH, 1);
                selectedCalender.set(Calendar.MONTH, Integer.parseInt(str) - 1);
                currentMon = str; //保存选择的月份
                dayChange();
            }
        });
        ...
        day_pv.setOnSelectListener(new DatePickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                String str = StringFormatUtils.delYDM(text);
                selectedCalender.set(Calendar.DAY_OF_MONTH, Integer.parseInt(str));
                currentDay = str;//保存选择的日期
                hourChange();
            }
        });
