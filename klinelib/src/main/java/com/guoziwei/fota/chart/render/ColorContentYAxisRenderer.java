...
public class ColorContentYAxisRenderer extends YAxisRenderer {
    ...
    public IMarker getmXMarker() {
        return mXMarker;
    }

    public void setmXMarker(IMarker mXMarker) {
        this.mXMarker = mXMarker;
    }
...
//                xPos += Utils.convertDpToPixel(15);
            } else {
                mAxisLabelPaint.setTextAlign(Paint.Align.RIGHT);
                xPos = mViewPortHandler.contentRight() - xoffset;
            }
...
//                    y = y - yoffset + textHeight + space+1F;
            ...
            //jiang 1116
            if (mYAxis.getAxisDependency() == YAxis.AxisDependency.RIGHT) {
                float textWidth = Utils.calcTextWidth(mAxisLabelPaint, text);
                if(fixedPosition + textWidth >= mViewPortHandler.getChartWidth()) {
                    float delta = fixedPosition + textWidth - mViewPortHandler.getChartWidth();
                    fixedPosition -= (delta + 10);
                }
            }
    ...
    public void renderLimitLines(Canvas c) {
        ...
        for (int i = 0; i < limitLines.size(); i++) {
            ...
            drawHoldMarker(c, pts[1]);
        }
    }

    private void drawHoldMarker(Canvas canvas, float y) {
        if(mXMarker == null) {
            return;
        }

        float[] pos = new float[]{0, y};

        // check bounds
        if (!mViewPortHandler.isInBounds(pos[0], pos[1]))
            return;

        mXMarker.refreshContent(null, null);
        LineChartXMarkerView xMarker = (LineChartXMarkerView) mXMarker;
//        int width = yMarker.getMeasuredWidth();

        mXMarker.draw(canvas, pos[0] + Utils.convertDpToPixel(4), y - xMarker.getMeasuredHeight());
    }
