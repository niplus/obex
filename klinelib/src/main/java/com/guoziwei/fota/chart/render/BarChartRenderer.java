...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for(int j = 0; j < buffer.size(); j += 4) {
            ...
            if(hisData != null && hisData.getVol() > 0) {
                needDraw = true;
                break;
            }
        }
...
//                mRenderPaint.setColor(dataSet.getColor(j / 4));
                ...
                if (data == null || !(data instanceof HisData)) {
                    ...
                    //jiang add 0.5
                    if(hisData.getVol() == 0) {
                        float originalStroke = mRenderPaint.getStrokeWidth();
                        mRenderPaint.setStrokeWidth(1f);
                        c.drawLine(
                                buffer.buffer[j], buffer.buffer[j + 3], buffer.buffer[j + 2],
                                buffer.buffer[j + 3],
                                mRenderPaint);
                        mRenderPaint.setStrokeWidth(originalStroke);
                        if (drawBorder) {
                            c.drawRect(buffer.buffer[j], buffer.buffer[j + 3], buffer.buffer[j + 2],
            ...
            //jiang fix 很小的值，volume柱形图根本画不出来
            if(Math.abs(top - bottom) > 1f) {
    ...
    protected Path mHighlightPath = new Path();
...
//            c.drawRect(mBarRect, mHighlightPaint);
            ...
            mHighlightPaint.setPathEffect(new DashPathEffect(new float[] {
                    5f, 5f
            }, 0f));
//            c.drawLine(mBarRect.centerX(), mViewPortHandler.getContentRect().bottom, mBarRect.centerX(), 0, mHighlightPaint);
            mHighlightPath.reset();
            //jiang
            mHighlightPath.moveTo(mBarRect.centerX(), mViewPortHandler.getContentRect().bottom);
            mHighlightPath.lineTo(mBarRect.centerX(), mViewPortHandler.getContentRect().top);
